...
function objetoProductoActual(producto) {
  ...
  //totalhtml representa el total en el html, y de esta insertamos dinamicamente el total
  totalhtml.innerHTML = `
  <span class="total">${total}</span>
  `;

  //antes agregar al carrito debemos verificar si ya estaba previameamente en la lista, si ya lo está solamente se debe sumar uno a la cantidad, sino pues agregarlo normalmente.
  //.some es la forma de iterar en un arreglo para buscar si este contiene un elemento
  ...
  if (existe) {
    ...
    const nuevoarreglo = articuloscarrito.map((producto) => {
  ...
  //Se debe llamar la funcion carritoHtml desde está funcion para que pueda tomar los valores del objeto correctamente
  carritoHtml();
}
...
function eliminarUnElemento(e) {
  ...
  `;
    ...
    //generamos nuevamente el while que es la forma mas optima para eliminar html de esa fila seleccionada
